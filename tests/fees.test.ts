...
test("celkový výpočet — riadky z tabuľky (presné centy)", () => {
  // E77: 87,30 € → app 5, provízia 13,10, spolu 18,10
  ...
  // E13: 135,60 € → app 10, provízia 20,34, spolu 30,34
  const e13 = vypocitajPoplatky(135.6);
  assert.equal(e13.poplatokApp, 10);
  assert.equal(e13.provizia, 20.34);
  assert.equal(e13.celkovyPoplatok, 30.34);

  // E21: 390,90 € → app 20, provízia 58,64, spolu 78,64
  const e21 = vypocitajPoplatky(390.9);
  assert.equal(e21.poplatokApp, 20);
  assert.equal(e21.provizia, 58.64);
  assert.equal(e21.celkovyPoplatok, 78.64);

  // E10: 701,10 € → app 20, provízia 105,17, spolu 125,17
  const e10 = vypocitajPoplatky(701.1);
  assert.equal(e10.poplatokApp, 20);
  assert.equal(e10.provizia, 105.17);
  assert.equal(e10.celkovyPoplatok, 125.17);
